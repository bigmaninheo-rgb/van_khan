...
def fix_final_content(prayer_num):
    ...
    # Tìm và thay thế các định dạng sai
    content = re.sub(
        r'ngày \d+\/[A-Za-z]+',
        amlich,
        content
    ...
    # Tìm và thay thế "Ngày âm lịch: ngày 31/Ba" -> "Ngày âm lịch: 13/02/2026"
    content = re.sub(
        r'Ngày âm lịch: ngày \d+\/[A-Za-z]+',
        f'Ngày âm lịch: {amlich}',
        content
    )
    
    # 2. THÊM "....." VÀO CÁC CHỖ TRỐNG
    # Tìm các chỗ trống sau dấu ":" hoặc "là" và thêm "....."
    lines = content.split('\n')
    for i, line in enumerate(lines):
        # Tìm các pattern trống
        if 'là ' in line and line.strip().endswith('là '):
            lines[i] = line + '.....'
        elif 'tên là ' in line and line.strip().endswith('tên là '):
            lines[i] = line + '.....'
        elif 'ngụ tại: ' in line and line.strip().endswith('ngụ tại: '):
            lines[i] = line + '.....'
        elif 'sinh ngày ' in line and line.strip().endswith('sinh ngày '):
            lines[i] = line + '.....'
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {{
                ...
                if (prayerText) {{
                    ...
                    // Sửa các định dạng ngày âm lịch sai
